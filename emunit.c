...
void ut_assert_delta(
	...
	emunit_num_t actual)
{
	bool result; /* Range check result, true if in range */
	if(emunit_numtype_signed_check(p_head->numtype))
	{
		...
		result = (min <= actual.s) && (actual.s <= max);
	}
	else
	{
		...
		result = (min <= actual.u) && (actual.u <= max);
	}
	if(!result)
	{
	...
	emunit_num_t actual)
{
	...
	if(emunit_numtype_signed_check(p_head->numtype))
	{
		result = (min.s <= actual.s) && (actual.s <= max.s);
	}
	else
	{
		result = (min.u <= actual.u) && (actual.u <= max.u);
	}
	if(!result)
	{
